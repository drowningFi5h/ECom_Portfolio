...
import { parseSize, parsePackQty } from '@/lib/amazon-costs';
...
async function getData() {
  ...
  const allSizeMap = new Map<string, { h: number; b: number; l: number }>();
  const packSet    = new Set<number>();

  const rows = (invRes.data ?? []).map(inv => {
    const size     = parseSize(inv.seller_sku);
    const size_key = size ? `${size.h}x${size.b}x${size.l}` : null;
    if (size && size_key) allSizeMap.set(size_key, size);
    const pack_qty_parsed = parsePackQty(inv.product_name);
    packSet.add(pack_qty_parsed);
  ...
  const allSizes = [...allSizeMap.entries()]
    .sort(([, a], [, b]) => a.h * a.b * a.l - b.h * b.b * b.l)
    .map(([key]) => key);

  const allPacks = [...packSet].sort((a, b) => a - b);

  const tablesMissing = costRes.error?.code === '42P01';
  const feesSynced = !feeRes.error && (feeRes.data?.length ?? 0) > 0;
  return { rows, tablesMissing, feesSynced, allSizes, allPacks };
}

export default async function CostsPage() {
  const { rows, tablesMissing, feesSynced, allSizes, allPacks } = await getData();
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      ...
      {tablesMissing ? (
        ...
        <>
          ...
          <CostsTable rows={rows} allSizes={allSizes} allPacks={allPacks} />
        </>
      )}
    </div>
  );
